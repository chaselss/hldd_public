			...
			<div class="registerbody">
				...
				<form>
					<div class="tel">
						...
						<input type="text" placeholder="请输入您的手机号码" v-model="telText"/>
					</div>
					<div class="pas">
				...
				<p class="join" @click="toLogin()"><span>现在注册</span><span>忘记密码？</span></p>
			</div>
		</section>
		<footer></footer>
	</div>
				 ...
				 if(this.telText!=''){
					if(this.psdText!=""){
						if( /^[0-9,a-z,A-Z]{6,16}$/.test(this.psdText) ){
						    ...
						    var md55 = this.md5(this.md5(this.psdText)+timespan) 
				            params.append('username', this.telText);
				            params.append('sign', md55);
				            params.append('timespan', timespan);
				            params.append('client', 'wap');
				            params.append('client_id', '');
				            params.append('country_code', '+86');
				            axios.post(`/api/api/index.php?c=login&a=indexnew&from=mobile`,params,{
				                  headers:{
				                     'content-type':'application/x-www-form-urlencoded',
				                     charset:'UTF-8'
